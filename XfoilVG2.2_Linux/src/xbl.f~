...
C ### BEGIN: Define source term strength
C ### DELPHINE/DANIEL CODE MODIFICATIONS - TIMESTAMP: 7 NOVEMBER 2017
C
      VGTHET = 0.0
      VGDSTR = 0.0
...
C Calculate velocity at VG using Swafford profile       
        cf = (0.3*exp(-1.33*VGH))/((log10(VGRT))**(1.74+0.31*VGH)) 
        cf = cf + 1.1E-4*(tanh(4-VGH/0.875)-1)      
        sss = sign(1.0,cf)
        ue_plus = (2.0/abs(cf))**(0.5)
        y_plus = VGRT/ue_plus*H_VG/VGTHET
        u_ue_2 = 1.0/1.95*(atanh((8.5-VGH)/7.5)-0.364)
        u_ue_5 = 0.155+0.795/cosh(0.51*(VGH-1.95))
        g_2 = (u_ue_2 - sss/(0.09*ue_plus)*atan(0.18*VGRT/ue_plus))
        g_2 = g_2/(1.0-sss*3.141592/(0.18*ue_plus))
        g_5 = (u_ue_5 - sss/(0.09*ue_plus)*atan(0.45*VGRT/ue_plus))
        g_5 = g_5/(1.0-sss*3.141592/(0.18*ue_plus))
        b = log(atanh(g_2**2.0)/atanh(g_5**2.0))/log(2.0/5.0)
        a = atanh(g_2**2.0)/2.0**b
        u_plus = sss/0.09*atan(0.09*y_plus)
        temp = (ue_plus-sss*3.141592/0.18)
        u_plus = u_plus+temp*(tanh(a*(H_VG/VGTHET)**b))**0.5
        UUEVG = u_plus/ue_plus*VGUE;
        
C        write(*,*) VGRT, VGH, VGTHET, VGUE, H_VG, UUEVG
...
C Define Integral according to emperical relation
        TRZVG = B_head(1)
        TRZVG = TRZVG * (H_VG/W_VG)**B_head(2)
        TRZVG = TRZVG * PARVG**B_head(3)
...
C Fix source term decay
        DECAYVG = 50.0
          
C Compute source term strength
        DO loc = IBL-1, NBL(IS)-1
          IBLL = IBL-1
          ...
          ELSE
            locA = IPAN(loc,IS)
            PARTA = EXP(-DECAYVG*(X(locA)-XVG(IS)))*(X(locA)-XVG(IS))
          ENDIF
          locB = IPAN(loc+1,IS)
          PARTB = EXP(-DECAYVG*(X(locB)-XVG(IS)))*(X(locB)-XVG(IS))
          TEMPVG = TEMPVG+(PARTA+PARTB)*(X(locB)-X(locA))
        ENDDO

         STRENGTHVG = TRZVG*EXP(-1.0)*2.0/(DECAYVG*TEMPVG)
         STRENGTHVG = nint(STRENGTHVG*100000.0)*1E-5
         XXX = 1
C Write the result
C        write(*,*) XXX
      ENDIF
 
C Compute value of the sourceterm at x position
      IF (X(iii) > XVG(IS)) THEN
         XL = X(iii)-XVG(IS)
         SOURCEVG = STRENGTHVG/(EXP(-1.0)*1.0/DECAYVG)
         SOURCEVG = SOURCEVG*EXP(-DECAYVG*XL)*XL
      ELSE 
         SOURCEVG = 0.0
      ENDIF
   
C Save source term for x position
      SVG(IBL,IS) = SOURCEVG
C
C ### DELPHINE/DANIEL CODE MODIFICATIONS - TIMESTAMP: 7 NOVEMBER 2017
C ### END: Define source term strength
C ### DELPHINE/DANIEL CODE MODIFICATIONS - TIMESTAMP: 7 NOVEMBER 2017
C
